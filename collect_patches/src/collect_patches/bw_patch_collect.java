...
public class bw_patch_collect {
	...
	public static void toCSV(double[][] array) throws IOException {
		...
		for (int i = 0; i < copied.length; i++) {
			for (int j = 0; j < copied[i].length; j++) {
				builder.append(copied[i][j]+"");
				if(j < copied[i].length - 1)
					builder.append(",");
			}
			builder.append("\n");
		}
	...
	public static double[][] get_patches(int nPatches, int pWidth, String filePath){
		...
		for(int i = 0; i < imageWidth; i++) {
			for(int j = 0; j < imageHeight; j++) {
				...
				imageArray[i][j] = Math.round(0.3*r + .59*g + .11*b) / 255.0;
				//UPDATE THIS LATER FOR TRUE GREYSCALE
			}
		}
		
		System.out.println(findMin(imageArray) + ", " + findMax(imageArray));
...
//				}
	...
	public static void showPatches(double[][] prePatches, int showPatchNum, boolean display, String name) {
		...
		for(int i = 0; i < showPatchNum; i++) {
			...
			if(pmax > pmin) {	
				for(int k = 0; k < patch.length; k++) {
					double check = (patch[k] - pmin) / (pmax - pmin);	//convert values so they range from 0 to 1
					patch[k] = check;
				}
			}
		...
		if(display) {
			...
			try {
				ImageIO.write(buff, "png", new File(name + ".png"));	//save image to project directory
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	public static void main(String[] args) throws Exception {
		...
		double [][] pre_patches = get_patches(num_patch, patch_width, "/images/trees.jpg");
		
		//toCSV(pre_patches);
		String pre = "pre-patches_" + num_patch + "_" + patch_width + "_" + comps; 
		showPatches(pre_patches, 9, true, pre);
		
		FastICA fastICA = new FastICA();
		fastICA.fit(transpose(pre_patches), comps);
		
		double[][] mixing = fastICA.getEM();
			
		String post = "post-patches_" + num_patch + "_" + patch_width + "_" + comps; 
		showPatches(transpose(mixing), 9, true, post);
		...
		System.out.println(mixing.length + ", " + mixing[0].length);
